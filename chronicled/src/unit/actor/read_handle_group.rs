use super::Envelope;
use super::read_handle_actor::ReadActor;
use crate::storage::level_iterator::LevelIterator;
use chronicle_proto::pb_ext::{FetchEventsRequest, FetchEventsResponse};
use tonic::Status;

pub struct ReadHandleGroup {
    actors: Vec<ReadActor>,
    actor_id_mask: i64,
}

impl ReadHandleGroup {
    pub fn new(actor_num: usize, inflight_num: usize, reader: LevelIterator) -> Self {
        let mut actors = Vec::with_capacity(actor_num);
        let inflight_per_actor = inflight_num / actor_num;

        for index in 0..actor_num {
            let actor = ReadActor::new(index as i32, inflight_per_actor, reader.clone());
            actors.push(actor);
        }

        Self {
            actors,
            actor_id_mask: (actor_num - 1) as i64,
        }
    }

    #[inline]
    pub async fn dispatch(
        &self,
        envelope: Envelope<FetchEventsRequest, FetchEventsResponse, Status>,
    ) -> Result<(), Status> {
        let event = &envelope.request;
        let idx = (event.timeline_id & self.actor_id_mask) as usize;
        self.actors[idx]
            .send(envelope)
            .await
            .map_err(|_| Status::unavailable("read handle actor is unavailable."))
    }
}
