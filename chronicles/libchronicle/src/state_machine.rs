...
) -> Result<(Arc<Mutex<ReplicationState>>, Vec<tokio::task::JoinHandle<()>>), ChronicleError> {
    let state = Arc::new(Mutex::new(ReplicationState {
        timeline_id,
        term,
        lrs,
        lra,
...
pub(crate) async fn new_term(
    catalog: &Catalog,
    conns: &HashMap<String, Conn>,
    timeline_name: &str,
    tc: &chronicle_proto::pb_catalog::TimelineCatalog,
) -> Result<(i64, i64, Vec<Segment>, Segment), ChronicleError> {
    ...
    Ok((new_term, lra, new_segments, writable_segment))
}
